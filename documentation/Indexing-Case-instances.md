...
After enabling case federation, the Federated Data Repository index rebuild will need to be triggered by an administrator or operator for the historical case instances to be indexed. The first indexing must be performed before a case management system is registered to IBM Process Federation Server. Subsequent full reindexing is rarely needed, but you can initiate full reindexing if you find that the index is not up to date. Case instances with the following properties are indexed:
