...
1. Open a terminal on a container running within an IBM Process Federation Server pod from the Console UI of your Openshift cluster or using CLI:
   ```
...
1. List the existing indexes:
   ...
   ```
   Replace _<FederatedDataRepository_Username>_ and _<FederatedDataRepository_Password>_ with the Federated Data Repository username and password.
...
1. Delete the index containing data about a given federated system:
   ...
   ```
   After the deletion of the index, IBM Process Federation Server automatically rebuilds it.
