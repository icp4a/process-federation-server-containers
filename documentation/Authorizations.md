# Specifying IBM Process Federation Server user authorizations on Kubernetes

When IBM Process Federation Server is installed, the operator defines a default set of authorizations for IBM Process Federation Server users, that you can customize depending on your needs.

## Default authorizations for IBM Process Federation Server users

The following attributes of the `ProcessFederationServer` custom resource define the administrator or the group of administrators to be used by the operator when setting the default authorizations:
* [admin_user_id](https://www.ibm.com/docs/cloud-paks/cp-biz-automation/25.0.0?topic=reference-cp4ba-process-federation-server-parameters): id(s) of the administrator user(s)
* [admin_group_id](https://www.ibm.com/docs/cloud-paks/cp-biz-automation/25.0.0?topic=reference-cp4ba-process-federation-server-parameters): id(s) of the administrator group(s)
...
The following table lists the default authorizations that are sets for IBM Process Federation Server users when running in Kubernetes:
...
For more information about the different roles and the corresponding granted authorizations, see [Federated systems and authorization for saved searches](https://www.ibm.com/docs/baw/25.0.0?topic=portal-federated-systems-authorization-saved-searches).

## Defining your own set of authorizations

The syntax to reference a group or a user in authorizations differs depending on your deployment of IBM Process Federation Server containers:
...
#### 1. Create a Kubernetes secret that contains a `.xml` file to be mounted as a configuration drop-in for the IBM Process Federation Server server (for example, use the key _authorizations.xml_ in your secret):
...
The authorization roles can be customized to provide role based access control to the REST APIs exposed by PFS.
...
* You can also use `<special-subject type="ALL_AUTHENTICATED_USERS"/>` entries to assign a role to all IBM Process Federation Server users.

  
#### 1.2. Liberty `administrator-role`

The Liberty administrator role provides read and write access to all JMX methods and MBeans:
- Use `user-access-id` and `group-access-id` to grant users and groups the administrative role
...
pfs_configuration:
  ...
  This step ensures that no default authorizations are defined for the IBM Process Federation Server server by the `ProcessFederationServer` custom resource, and that all the `.xml` files defined in the Kubernetes secret `<name-of-your-secret>`, including the one that defines the authorizations, will be mounted in the config dropins directory of the IBM Process Federation Server server, and properly added to the IBM Process Federation Server server configuration.
